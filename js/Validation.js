class Validation {
	...
	validate(e) {
		...
		if(elFields[fieldName].required) {
			if(fieldValue === '') {
				this.errors[fieldName].push('Please fill this field');
			}
		}

		if(elFields[fieldName].email) {
			if(!this.validateEmail(fieldValue)) {
				this.errors[fieldName].push('Wrong email');
			}
		}

		if(fieldValue.length < elFields[fieldName].minlength || fieldValue.length > elFields[fieldName].maxlength) {
			this.errors[fieldName].push(`Min  ${elFields[fieldName].minlength} and max ${elFields[fieldName].maxlength} characters`);
		}
		...
		if(elFields[fieldName].matching) {
			...
			if(fieldValue !== matchingEl.value) {
				this.errors[fieldName].push('Wrong password');
			}
